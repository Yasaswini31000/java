...
public class MortgageCalEx2 {
    final static byte MONTHS_IN_YEAR=12;
    final static byte PERCENT=100;
    public static void main(String[] args) {

        int principal=(int)readInput("Principal",1000,10000000);
    ...
    public static void printPaymentSchedule(short noOfPayments,int principal,float monthlyRate) {
        ...
        for(short month=1;month<=noOfPayments;month++) {
            double balance=principal*(Math.pow((1+monthlyRate),noOfPayments)
                    - Math.pow((1+monthlyRate),month))/(Math.pow((1+monthlyRate),noOfPayments)-1);
